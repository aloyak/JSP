...
SandboxMode::SandboxMode(Game &game)
    : GameMode("Sandbox", "assets/scenes/menu.scene"), m_game(game) {}
...
Vec3 SandboxMode::GetMouseIntersection()
{
...
bool SandboxMode::IsPositionValid(const Vec3 &pos, float ghostRadius)
{
...
const GravityBody *SandboxMode::FindBodyDef(const std::string &name) const
{
    return m_registry.find(name);
}

void SandboxMode::SelectPlanet(Entity *planet)
{
    if (!planet)
        return;
...
bool SandboxMode::WorldToScreen(const Vec3 &worldPos, ImVec2 &outScreen) const
{
...
void SandboxMode::StartPlacement(int index)
{
...
void SandboxMode::ConfirmPlacement()
{
    ...
    if (m_reallocatingExisting)
    {
        ...
        if (finishedRc)
            finishedRc->setBaseColor(m_preReallocColor);
    }
...
void SandboxMode::CancelPlacement()
{
    if (m_ghostEntity)
    {
        if (m_reallocatingExisting)
        {
            ...
            if (existingRc)
                existingRc->setBaseColor(m_preReallocColor);
            planetList.push_back(m_ghostEntity);
        }
        else
        {
...
void SandboxMode::HandlePlacementLogic()
{
    ...
    case ToolMode::Reallocation:
        if (m_placeState == PlaceState::None && leftJustPressed)
        {
            ...
            if (idx >= 0)
            {
                ...
                auto *existingRc = m_ghostEntity->getComponent<RenderComponent>();
                if (existingRc)
                    m_preReallocColor = existingRc->getBaseColor();
                m_placeState = PlaceState::Positioning;
            }
        }
        break;
    ...
    case ToolMode::Velocity:
        ...
        else
        {
            ...
            if (!leftPressed)
            {
                ...
                if (planetComp)
                    planetComp->getPlanetParams().velocity = velocity;
...
void SandboxMode::DrawVelocityArrow()
{
    if (m_toolMode != ToolMode::Velocity || !m_velDragging || !m_velocityTarget)
        return;

    ImVec2 screenOrigin;
    if (!WorldToScreen(m_velDragWorldOrigin, screenOrigin))
        return;
...
void SandboxMode::DrawAllVelocityVectors()
{
    if (m_toolMode != ToolMode::Velocity)
        return;
    ...
    for (Entity *planet : planetList)
    {
        if (!planet)
            continue;
        if (m_velDragging && planet == m_velocityTarget)
            continue;
        ...
        ImVec2 screenOrigin;
        if (!WorldToScreen(worldOrigin, screenOrigin))
            continue;
...
void SandboxMode::DrawSelectionHighlight()
{
    if (m_toolMode != ToolMode::Selection || !selectedEntity)
        return;

    ImVec2 screenPos;
    if (!WorldToScreen(selectedEntity->transform.position, screenPos))
        return;
...
void SandboxMode::UpdateFocusTransition(float dt)
{
...
void SandboxMode::DestroyPlanet(Entity *planet)
{
    ...
    if (it != planetList.end())
        planetList.erase(it);
}

void SandboxMode::UpdateDestroyAnimations(float dt)
{
    ...
    m_destroyAnims.erase(
        std::remove_if(m_destroyAnims.begin(), m_destroyAnims.end(),
                       [](const DestroyAnimation &a)
                       { return a.done; }),
        m_destroyAnims.end());
}

void SandboxMode::DrawDestroyAnimations()
{
    if (m_destroyAnims.empty())
        return;
...
void SandboxMode::StartSimulation()
{
    m_simSnapshot.clear();
    for (Entity *planet : planetList)
    {
...
void SandboxMode::PauseSimulation()
{
    if (!simulationRunning)
        return;
    physicsPaused = true;
...
void SandboxMode::ResumeSimulation()
{
    if (!simulationRunning)
        return;
    physicsPaused = false;
...
void SandboxMode::ResetSimulation()
{
    if (!m_hasSnapshot)
        return;
    ...
    for (auto &snap : m_simSnapshot)
    {
        ...
        if (!alive)
            planetList.push_back(snap.entity);
    }
    m_trails.clear();
}

void SandboxMode::StepSimulation(float dt)
{
    ...
    for (int i = 0; i < (int)bodies.size(); ++i)
    {
        for (int j = i + 1; j < (int)bodies.size(); ++j)
        {
            ...
            if ((bodies[j].pos - bodies[i].pos).length() >= threshold)
                continue;
            ...
            if (std::find(toDestroy.begin(), toDestroy.end(), killEnt) != toDestroy.end())
                continue;
            if (std::find(toDestroy.begin(), toDestroy.end(), survEnt) != toDestroy.end())
                continue;

            float totalMass = bodies[i].mass + bodies[j].mass;
            Vec3 newVel = (bodies[i].vel * bodies[i].mass + bodies[j].vel * bodies[j].mass) * (1.0f / totalMass);
...
void SandboxMode::DrawTrails()
{
    if (!drawTrails || !simulationRunning || m_trails.empty())
        return;
...
void SandboxMode::Update()
{
    ...
    if (movingEnabled)
    {
        ...
        if (forwardXZ.length() < 0.0001f)
            forwardXZ = Vec3(0, 0, 1);

        Vec3 rightXZ = Vec3(forwardXZ.z, 0.0f, -forwardXZ.x);

        Vec3 movement(0, 0, 0);
        if (m_input.isKeyDown(KEY_W) && m_inputEnabled)
        {
            movement += forwardXZ;
            m_followingPlanet = false;
        }
        if (m_input.isKeyDown(KEY_S) && m_inputEnabled)
        {
            movement -= forwardXZ;
            m_followingPlanet = false;
        }
        if (m_input.isKeyDown(KEY_A) && m_inputEnabled)
        {
            movement += rightXZ;
            m_followingPlanet = false;
        }
        if (m_input.isKeyDown(KEY_D) && m_inputEnabled)
        {
            movement -= rightXZ;
            m_followingPlanet = false;
        }
...
void SandboxMode::SaveSandbox(const std::string &name)
{
    ...
    for (Entity *planet : planetList)
    {
        if (!planet || planet == m_centralBody)
            continue;
...
void SandboxMode::LoadSandbox(const std::string &filepath)
{
    ...
    for (const auto &entry : j["planets"])
    {
        ...
        if (entry.value("isReal", false))
        {
            ...
            for (int i = 0; i < (int)m_registry.bodies.size(); i++)
            {
                if (m_registry.bodies[i].name == regName)
                {
                    idx = i;
                    break;
                }
...
void SandboxMode::DrawSaveDialog()
{
    ...
    std::string title = std::string(m_ui.getText("sbox.save")) + "###SaveSandbox";
    ImGui::Begin(title.c_str(), &m_showSaveDialog,
                 ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove);

    ImGui::Text("%s", m_ui.getText("sbox.name"));
...
void SandboxMode::Transition(float dt)
{
    if (!m_orbitCamera)
        return;
...
void SandboxMode::DrawGrid(Renderer &renderer, Entity *m_camera)
{
    for (int i = -13; i <= 13; ++i)
    {
...
void SandboxMode::DrawUI()
{
    if (isTransitioning)
    {
        m_sandboxWindowActive = false;
        return;
    }
    ...
    std::string sandboxTitle = std::string(m_ui.getText("mm.sandbox")) + "###Sandbox";
    ImGui::Begin(sandboxTitle.c_str(), nullptr, flags);
    ...
    if (!ImGui::IsWindowCollapsed())
    {
        if (ImGui::BeginTabBar("SandboxTabBar", ImGuiTabBarFlags_NoTabListScrollingButtons))
        {
            ...
            if (m_ui.beginTabItem("sbox.tabs.planets"))
            {
                ...
                {
                    bool show = false;
                    if (selectedType == 0 && m_registry.bodies[i].isReal && !m_registry.bodies[i].isCustom)
                        show = true;
                    if (selectedType == 1 && !m_registry.bodies[i].isReal && !m_registry.bodies[i].isCustom)
                        show = true;
                    if (selectedType == 2 && m_registry.bodies[i].isCustom)
                        show = true;
            ...
            if (m_ui.beginTabItem("sbox.tabs.properties"))
            {
                if (selectedEntity && selectedEntity->getComponent<PlanetComponent>())
                {
                    ...
                    std::string massFmt = std::string(m_ui.getText("mass")) + ": %.3e";
                    ImGui::SetNextItemWidth(halfWidth);
                    if (ImGui::SliderFloat("##MassField", &mass, 1.0f, 1.0e10f, massFmt.c_str(), ImGuiSliderFlags_Logarithmic))
                    {
                        pc->getPlanetParams().mass = mass;
                    }

                    ImGui::SameLine();
                    std::string radiusFmt = std::string(m_ui.getText("radius")) + ": %.1f";
                    ImGui::SetNextItemWidth(halfWidth);
                    if (ImGui::SliderFloat("##RadiusField", &radius, 1.0f, 100000.0f, radiusFmt.c_str(), ImGuiSliderFlags_Logarithmic))
                    {
                        pc->getPlanetParams().radius = radius;
                        pc->applyScale();
                    }

                    std::string periodFmt = std::string(m_ui.getText("period")) + ": %.3f";
                    ImGui::SetNextItemWidth(halfWidth);
                    if (ImGui::SliderFloat("##PeriodField", &period, 0.0f, 1000.0f, periodFmt.c_str()))
                    {
                ...
                else
                {
                    ...
                    m_ui.setFont(1);
                    float titleWidth = ImGui::CalcTextSize(m_ui.getText("sbox.prpts.nps1")).x;
                    ImGui::SetCursorPosY(ImGui::GetCursorPosY() * 0.5f + 60.0f);
                    ImGui::SetCursorPosX(halfWidth - titleWidth * 0.5f);
                    ImGui::TextDisabled("%s", m_ui.getText("sbox.prpts.nps1"));
                    m_ui.resetFont();
                    float subtitleWidth = ImGui::CalcTextSize(m_ui.getText("sbox.prpts.nps2")).x;
                    ImGui::SetCursorPosX(halfWidth - subtitleWidth * 0.5f);
                    ImGui::TextDisabled("%s", m_ui.getText("sbox.prpts.nps2"));
                }
            ...
            if (m_ui.beginTabItem("sbox.tabs.sttngs"))
            {
                ...
                std::string tsFmt = std::string(m_ui.getText("sbox.timescale")) + ": %.2f";
                ImGui::SliderFloat("##TimeScale", &m_game.timeScale,
                                   1.0f, 100000.0f, tsFmt.c_str(),
                                   ImGuiSliderFlags_Logarithmic);
                ImGui::SetNextItemWidth(totalWidth);
                std::string msFmt = std::string(m_ui.getText("sbox.massscale")) + ": %.3fx";
                ImGui::SliderFloat("##MassScale", &m_massScale,
                                   0.01f, 100.0f, msFmt.c_str(),
                                   ImGuiSliderFlags_Logarithmic);
                ImGui::EndTabItem();
            }
...
void SandboxMode::DrawMainMenuBar()
{
    if (isTransitioning)
    {
    ...
    if (ImGui::BeginMainMenuBar())
    {
        ...
        if (m_ui.beginMenu("sbox.tabs.planets"))
        {
            ...
            if (planetList.empty())
            {
                ImGui::TextDisabled("%s", m_ui.getText("sbox.npp"));
            }
            else
            {
                for (Entity *planet : planetList)
        ...
        const char *statusText = simulationRunning ? (physicsPaused ? m_ui.getText("sbox.sim.paused") : m_ui.getText("sbox.sim.running")) : "";
        ...
        {
            toolName = physicsPaused ? m_ui.getText("sbox.sim.paused") : m_ui.getText("sbox.sim.running");
        }
        else
        {
            toolName = (m_toolMode == ToolMode::Selection)      ? m_ui.getText("sbox.tool.select")
                       : (m_toolMode == ToolMode::Reallocation) ? m_ui.getText("sbox.tool.move")
                                                                : m_ui.getText("sbox.tool.velocity");
        }
...
void SandboxMode::DrawPlanetsWindow()
{
    ImGui::SetNextWindowSize(ImVec2(300, 400), ImGuiCond_FirstUseEver);
    std::string psTitle = std::string(m_ui.getText("sbox.planetsstars")) + "###PlanetsStars";
    if (ImGui::Begin(psTitle.c_str(), &showPlanetsWindow, ImGuiWindowFlags_NoCollapse))
    {
        if (planetList.empty())
        {
            ImGui::TextDisabled("%s", m_ui.getText("sbox.npp"));
