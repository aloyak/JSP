...
PlanetBuilderMode::PlanetBuilderMode(Game& game)
    : GameMode("PlanetBuilder", "assets/scenes/space.scene")
    , m_game(game)
    , m_input(game.GetEngine().getInput())
    , m_ui(game.GetUI())
    , m_audio(game.GetAudioManager())
    , m_selector(game, "user/planets", ".planet") {
}
...
void PlanetBuilderMode::OnExit() {
    StopBrushSound();
    m_paintTexture.reset();
    delete m_orbitCamera;
}

void PlanetBuilderMode::Update() {
...
void PlanetBuilderMode::LateUpdate() {
    ...
    if (ImGui::BeginMainMenuBar()) {
        ...
        if (m_ui.beginMenu("tm.pb.planet")) {
            if (m_ui.menuItem("tm.pb.new")) { 
                m_game.SetGameMode(std::make_unique<PlanetBuilderMode>(m_game), true);
            }
            if (m_ui.menuItem("tm.pb.save")) { Save(); }
            if (m_ui.menuItem("tm.pb.load")) { m_selector.toggleOpen(); }
            ImGui::EndMenu();
        }
        if (m_ui.beginMenu("tm.view")) {
            if (m_ui.menuItem("tm.properties")) { showProperties = !showProperties; }
            if (m_ui.menuItem("tm.tools"))      { showTools      = !showTools; }
            ImGui::EndMenu();
        }
...
void PlanetBuilderMode::DrawPropertiesWindow() {
    ...
    if (planet) {
        ...
        if (m_ui.checkbox("atms.has", &hasAtmosphere)) {
        ...
        if (m_ui.checkbox("wtr.has", &hasWater)) {
...
void PlanetBuilderMode::DrawToolsWindow() {
    ...
    if (ImGui::BeginTabBar(m_ui.getText("tm.tools"))) {
        ...
        if (m_ui.beginTabItem("pb.paint", nullptr, paintFlags)) {
            ...
            ImGui::SameLine();
            if (m_ui.checkbox("paint.brush.sound", &m_brushSoundEnabled)) {}
...
float PlanetBuilderMode::toLinear(float c) const {
...
void PlanetBuilderMode::PaintUVGradient() {
...
void PlanetBuilderMode::UpdateMouseIntersection() {
...
void PlanetBuilderMode::GetHoverUV(float& outU, float& outV) const {
...
void PlanetBuilderMode::StartBrushSound() {
...
void PlanetBuilderMode::DrawBrushIndicator() {
...
void PlanetBuilderMode::TryTerrain() {
...
void PlanetBuilderMode::ResetTerrain() {
    auto* renderComp = planet->getComponent<RenderComponent>();
    if (!renderComp) return;
    renderComp->reloadModel();
}

void PlanetBuilderMode::Save() {
...
void PlanetBuilderMode::Load(const std::string& filepath) {
    auto* planetComponent = planet->getComponent<PlanetComponent>();
    if (planetComponent) {
